...
void database_cheque(Database *database, unsigned long amount, unsigned long sender, unsigned long receiver, unsigned long reference){
	...
	client_received(tree_search(database->tree, receiver), amount);
...
void database_process(Database *database){
	...
	if (client_issue(tree_search(database->tree, cheque_sender(cheque)), -cheque_amount(cheque)))
		tree_remove(database->tree, cheque_sender(cheque));
	if (client_received(tree_search(database->tree, cheque_receiver(cheque)), -cheque_amount(cheque)))
